...
- Order pricing: `OrderItem.UnitPriceSnapshot` always freezes the product's *full, undiscounted* price at order-creation time — line/order totals are always computed from the snapshot, not the current `Product.UnitPrice`. Customer-tier discounts (`CustomerTier.Gold`/`Silver`/`Standard`) are applied exactly once, via `OrderService.GetDiscountRate` inside `CalculateTotal`.
